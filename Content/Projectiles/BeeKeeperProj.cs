...
{
    public class BeeKeeperProj : ModProjectile
    {
        ...
        {
            for (int i = 0; i < (int)Bees; i++)
                Projectile.NewProjectile(Projectile.GetSource_FromThis(), target.Center, new Vector2(Main.rand.Next(-1, 1), Main.rand.Next(-1, 1)), ProjectileID.Bee, 10, 0.1f);
        }
    }
}